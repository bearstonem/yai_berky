...
```shell
helm list all processes listening on port 8080
```

You can ask for a command line generation, enforcing `🚀 exec` prompt mode usage with `-e`:

```shell
helm -e show the disk usage of my docker resources
```

You can ask any question, enforcing `💬 chat` prompt mode usage with `-c`:

```shell
helm -c generate me a go application example using fiber
```

You can also `pipe` input that will be taken into account in your request:

```shell
cat some_script.go | helm -c generate unit tests
```

Or even:

```shell
cat error.log | helm -c explain what is wrong here
```
